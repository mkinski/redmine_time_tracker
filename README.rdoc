...
This version is only meant to be used with Redmine version 2.0 running on Rails 3.2.x, which has significant changes on routes.rb etc.
Some wiki pages still are not up to date so dont wonder yourself if some commands are not documented on the official site.
...
1. Follow the Redmine plugin installation steps at: http://www.redmine.org/wiki/redmine/Plugins                                                                       
   ...
   install to <tt>plugins/redmine_time_tracker</tt>
...
1. Downgrade the database using the migrations. 
